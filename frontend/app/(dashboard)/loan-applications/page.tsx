...
function getStatusVariant(status: string): 'success' | 'warning' | 'error' | 'info' | 'default' {
  ...
  if (normalizedStatus === 'UNDER_REVIEW' || normalizedStatus === 'UNDER REVIEW') return 'warning';
  return 'default';
}

function getStatusIcon(status: string) {
  const normalizedStatus = status?.toUpperCase();
  if (normalizedStatus === 'APPROVED') return <CheckCircle className="w-4 h-4" />;
  if (normalizedStatus === 'REJECTED' || normalizedStatus === 'CANCELLED') return <XCircle className="w-4 h-4" />;
  if (normalizedStatus === 'SUBMITTED') return <FileText className="w-4 h-4" />;
  if (normalizedStatus === 'UNDER_REVIEW' || normalizedStatus === 'UNDER REVIEW') return <Clock className="w-4 h-4" />;
...
export default function LoanApplicationsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const rejectedCount = applications.filter(
      (app) => app.status === 'Rejected' || app.status === 'REJECTED' || app.status === 'Cancelled' || app.status === 'CANCELLED'
    ).length;
    ...
    const processedApps = applications.filter(
      (app) => (app.status === 'Approved' || app.status === 'APPROVED' || app.status === 'Rejected' || app.status === 'REJECTED') && app.createdAt
    );
    const avgProcessingTime = processedApps.length > 0
      ? processedApps.reduce((sum, app) => {
          const created = new Date(app.createdAt);
          const processed = app.status === 'Approved' || app.status === 'APPROVED'
            ? (app as any).approvalDate ? new Date((app as any).approvalDate) : new Date()
            : (app as any).rejectionDate ? new Date((app as any).rejectionDate) : new Date();
          return sum + differenceInBusinessDays(processed, created);
        }, 0) / processedApps.length
      : 0;
    
    // Calculate average loan amount
    const avgLoanAmount = totalApplications > 0 && totalAmount > 0 && !isNaN(totalAmount) 
    ...
    const pendingApps = applications.filter(
      (app) => 
        (app.status === 'Submitted' || app.status === 'SUBMITTED' || app.status === 'Under Review' || app.status === 'UNDER_REVIEW') && app.createdAt
    );
    const avgPendingAge = pendingApps.length > 0
      ? pendingApps.reduce((sum, app) => sum + differenceInDays(new Date(), new Date(app.createdAt)), 0) / pendingApps.length
      : 0;
  ...
  const uniqueStatuses = useMemo(() => {
    const statuses = Array.from(new Set(applications.map((app) => app.status).filter(Boolean))).sort();
    return statuses;
  }, [applications]);
  ...
  const filteredApplications = useMemo(
    () => {
      let filtered = applications.filter(
        (app) =>
          ...
          (companies.find(c => c.id === app.companyId)?.name || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
          (loanProducts.find(p => p.id === app.loanProductId)?.name || '').toLowerCase().includes(searchTerm.toLowerCase())
      );
    ...
    [applications, searchTerm, statusFilter, companyFilter, productFilter, companies, loanProducts]
  );

  const { sortedData, handleSort, getSortDirection } = useSort(filteredApplications);
  ...
  const filteredStats = useMemo(() => {
    ...
    const filteredAvgAmount = filteredApplications.length > 0 && filteredTotalAmount > 0 && !isNaN(filteredTotalAmount)
        ...
        {!isLoading && applications.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
            ...
            <div className="rounded-lg border border-gray-200 bg-white shadow-sm hover:shadow transition-shadow p-4">
              ...
              <p className="text-2xl font-bold text-gray-900 leading-tight mb-1">{stats.totalApplications}</p>
            ...
            <div className="rounded-lg border border-gray-200 bg-white shadow-sm hover:shadow transition-shadow p-4">
              ...
              <p className="text-2xl font-bold text-gray-900 leading-tight mb-1">{stats.approvedCount}</p>
              <div className="flex items-center gap-1">
                <TrendingUp className="w-3 h-3 text-gray-500" />
                <p className="text-[10px] text-gray-500">{stats.approvalRate.toFixed(1)}% rate</p>
              </div>
            </div>
            ...
            <div className="rounded-lg border border-gray-200 bg-white shadow-sm hover:shadow transition-shadow p-4">
              ...
              <p className="text-[10px] text-gray-500">
                {stats.avgPendingAge > 0 ? `Avg ${Math.round(stats.avgPendingAge)}d` : 'N/A'}
              </p>
            </div>
            ...
            <div className="rounded-lg border border-gray-200 bg-white shadow-sm hover:shadow transition-shadow p-4">
              ...
              <p className="text-2xl font-bold text-gray-900 leading-tight mb-1">{stats.rejectedCount}</p>
              <p className="text-[10px] text-gray-500">
                {stats.totalApplications > 0 ? ((stats.rejectedCount / stats.totalApplications) * 100).toFixed(1) : 0}%
              </p>
            </div>
            ...
            <div className="rounded-lg border border-gray-200 bg-white shadow-sm hover:shadow transition-shadow p-4">
              <div className="flex items-start justify-between mb-2">
                <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Processing</p>
                <Activity className="w-4 h-4 text-gray-400 flex-shrink-0" />
              </div>
              <p className="text-2xl font-bold text-gray-900 leading-tight mb-1">
                {stats.avgProcessingTime > 0 ? Math.round(stats.avgProcessingTime) : 'N/A'}
              </p>
          ...
          {showFilters && (
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  >
                    ...
                    {companies.map((company) => (
                      <option key={company.id} value={company.id}>
                        {company.name} ({applications.filter((app) => app.companyId === company.id).length})
                      </option>
                    ))}
                  </select>
                </div>
                ...
                {/* Loan Product Filter */}
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-3">Loan Product</label>
                  <select
                    value={productFilter}
                    onChange={(e) => setProductFilter(e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all bg-white"
                  >
                    <option value="all">All Products ({loanProducts.length})</option>
                    {loanProducts.map((product) => (
                      <option key={product.id} value={product.id}>
                        {product.name} ({applications.filter((app) => app.loanProductId === product.id).length})
                      </option>
                    ))}
                  </select>
                </div>
          ...
          {isLoading ? (
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50">
                      <TableHead className="font-semibold text-gray-700 text-xs py-2 px-3">Application #</TableHead>
                      <TableHead className="font-semibold text-gray-700 text-xs py-2 px-3">Company</TableHead>
                      <TableHead className="font-semibold text-gray-700 text-xs py-2 px-3">Product</TableHead>
                      <SortableHeader
                      ...
                      <TableHead className="font-semibold text-gray-700 text-xs py-2 px-3">Rate</TableHead>
                      <SortableHeader
                      ...
                      <TableHead className="font-semibold text-gray-700 text-xs py-2 px-3">Age</TableHead>
                      <SortableHeader
                        onSort={() => handleSort('createdAt' as keyof typeof paginatedData[0])}
                        sortDirection={getSortDirection('createdAt' as keyof typeof paginatedData[0])}
                        className="font-semibold text-gray-700 text-xs py-2 px-3"
                      >
                        Created
                      </SortableHeader>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {paginatedData.map((app) => {
                      ...
                      const product = loanProducts.find(p => p.id === app.loanProductId);
                      const daysSinceCreated = app.createdAt ? differenceInDays(new Date(), new Date(app.createdAt)) : 0;
                      const isUrgent = daysSinceCreated > 7 && (app.status === 'Submitted' || app.status === 'SUBMITTED' || app.status === 'Under Review' || app.status === 'UNDER_REVIEW');
                      
                      return (
                        <TableRow
                          key={app.id}
                          onClick={() => window.location.href = `/loan-applications/${app.id}`}
                          className={`hover:bg-primary/5 cursor-pointer transition-colors ${isUrgent ? 'bg-yellow-50/50 border-l-2 border-l-yellow-400' : ''}`}
                        >
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <div className="flex flex-col min-w-0">
                                <span className="font-mono text-xs font-semibold text-gray-900 truncate">
                                  {app.applicationNumber || (app.id ? `${app.id.slice(0, 8).toUpperCase()}...` : 'N/A')}
                                </span>
                                {isUrgent && (
                          ...
                          <TableCell className="py-2 px-3">
                            <div className="flex items-center gap-1.5 min-w-0">
                              ...
                              <span className="text-xs text-gray-900 font-medium truncate max-w-[100px]" title={company?.name}>
                                {company?.name || 'N/A'}
                              </span>
                            </div>
                          </TableCell>
                          <TableCell className="py-2 px-3">
                            <div className="flex items-center gap-1.5 min-w-0">
                              ...
                              <span className="text-xs text-gray-900 font-medium truncate max-w-[100px]" title={product?.name}>
                                {product?.name || 'N/A'}
                              </span>
                            </div>
                          </TableCell>
                          <TableCell className="py-2 px-3">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-xs text-gray-900 font-medium">{app.applicantType || 'N/A'}</span>
                            </div>
                          </TableCell>
                          <TableCell className="py-2 px-3">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span 
                                ...
                                title={`$${getApplicationAmount(app).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`}
                              >
                          ...
                          <TableCell className="py-2 px-3">
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-xs text-gray-700 font-medium">
                                {app.rateOfInterest ? `${app.rateOfInterest}%` : 'N/A'}
                              </span>
                            </div>
                          </TableCell>
                          <TableCell className="py-2 px-3">
                            <Badge variant={getStatusVariant(app.status)} className="flex items-center gap-1 w-fit text-xs py-0.5 px-2">
                              <span className="scale-75">{getStatusIcon(app.status)}</span>
                              <span className="text-xs">{app.status || 'N/A'}</span>
                            </Badge>
                          </TableCell>
                          <TableCell className="py-2 px-3">
                            <div className="flex items-center gap-1">
                              <Clock className={`w-3 h-3 flex-shrink-0 ${isUrgent ? 'text-yellow-600' : 'text-gray-400'}`} />
                              <span className={`text-xs font-medium ${isUrgent ? 'text-yellow-600' : 'text-gray-600'}`}>
                                {daysSinceCreated}d
                              </span>
                            </div>
                          </TableCell>
            ...
            <div className="p-12">
              <EmptyState
                ...
                title={searchTerm || statusFilter !== 'all' ? "No applications found" : "No loan applications yet"}
                description={
                ...
                action={
                  !searchTerm && statusFilter === 'all'
                    ? {
                        ...
                        onClick: () => (window.location.href = '/loan-applications/new'),
                      }
                    : undefined
                }
              />
