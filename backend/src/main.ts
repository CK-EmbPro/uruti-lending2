...
Version: 1.0.0
    ...
    .addTag('loan-application-documents', 'Loan application document upload and verification')
    .addTag('document-types', 'Document type master data')
    // Loan Management
    .addTag('loans', 'Loan management operations (CRUD, closure, NPA, transfer, FLDG)')
    .addTag('loan-disbursements', 'Loan disbursement management')
    .addTag('loan-repayments', 'Loan repayment operations (17 types, bulk repayment)')
    ...
    .addTag('loan-security-assignments', 'Security assignment, pledging, and release')
    .addTag('loan-security-prices', 'Security price management and history')
    .addTag('loan-security-shortfalls', 'Security shortfall detection and resolution')
    .addTag('loan-security-deposit', 'Security deposit usage tracking')
    // Co-Lending
    .addTag('loan-partners', 'Co-lending partner management and FLDG configuration')
    ...
    // Reporting
    .addTag('reporting', 'Financial reporting (Portfolio, NPA, Collection, Disbursement, Overdue)')
    // Workflow
  ...
  app.useGlobalPipes(
    ...
    console.log(`🚀 Uruti Lending Platform API is running on: http://localhost:${port}/${apiPrefix}`);
